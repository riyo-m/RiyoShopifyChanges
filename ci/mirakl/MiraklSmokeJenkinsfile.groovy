node('e2e') {
    ...
    stage('smoke-test') {
        container('mvn3-jdk11') {
            try {
                ...
                sh """
          cd ConnectorQuality
          mvn clean test -Dgroups="mirakl_smoke" -e -Dservices.webdriver.provisioned=true -Dservices.webdriver.headless=true  -Dmaven.test.failure.ignore=true
        """
            } catch (e) {
    ...
    stage('Import Results') {
        step([$class: 'XrayImportBuilder', endpointName: '/junit', importFilePath: 'ConnectorQuality/target/surefire-reports/junitreports/TEST-com.vertex.quality.connectors.mirakl.api.tests.*xml', importToSameExecution: 'true', projectKey: 'MIR', serverInstance: 'CLOUD-47847935-53f5-4e62-b39a-16a3e619ed4a', testExecKey: 'MIR-75'])
    }
