node('e2e') {
    ...
    stage('endpoint-test') {
        container('mvn3-jdk11') {
            try {
                sh """
          cd ConnectorQuality
          mvn test -Dgroups=netsuite_ow_smoke -DconnectorId=4 -Denv="oneWorldUpgrade" -e -Dservices.webdriver.provisioned=true -Dservices.webdriver.headless=true
        """
            } catch (e) {
    ...
    stage('endpoint-test') {
        container('mvn3-jdk11') {
            try {
                sh """
          cd ConnectorQuality
          mvn test -Dgroups=netsuite_smoke -DconnectorId=5 -e -Dservices.webdriver.provisioned=true -Dservices.webdriver.headless=true
        """
            } catch (e) {
                // archiveArtifacts artifacts: 'ConnectorQuality/errorScreenshots/*.png',
                // currentBuild.result = 'FAILURE'
    ...
    stage('endpoint-test') {
        container('mvn3-jdk11') {
            try {
                sh """
          cd ConnectorQuality
          mvn test -Dgroups=netsuite_smoke -DconnectorId=6 -e -Dservices.webdriver.provisioned=true -Dservices.webdriver.headless=true
        """
            } catch (e) {
                // archiveArtifacts artifacts: 'ConnectorQuality/errorScreenshots/*.png',
                // currentBuild.result = 'FAILURE'
    ...
    stage('Import Results') {
        step([$class: 'XrayImportBuilder', endpointName: '/junit', importFilePath: 'ConnectorQuality/target/surefire-reports/junitreports/TEST-com.vertex.quality.connectors.netsuite.oneworld.tests.certification.*xml', importToSameExecution: 'true', projectKey: 'CNSL', serverInstance: 'CLOUD-47847935-53f5-4e62-b39a-16a3e619ed4a', testExecKey: 'CNSL-1392'])
    }
