node('e2e') {
    ...
    stage('smoke-test') {
        container('mvn3-jdk11') {
            try {
                sh """
          cd ConnectorQuality
          mvn test -Dgroups=netsuite_ow_smoke -e -Dservices.webdriver.provisioned=true -Dservices.webdriver.headless=true
        """
            } catch (e) {
    ...
    stage('Import Results') {
        step([$class: 'XrayImportBuilder', endpointName: '/junit', importFilePath: 'ConnectorQuality/target/surefire-reports/junitreports/TEST-com.vertex.quality.connectors.netsuite.oneworld.tests.*xml', importToSameExecution: 'true', projectKey: 'CNSL', serverInstance: 'CLOUD-47847935-53f5-4e62-b39a-16a3e619ed4a', testExecKey: 'CNSL-189'])
    }
    ...
    stage('invoke_regression_tests') {
        build job: 'NetSuiteLegacyOneworld_Regression', propagate: false, wait: false
    }
}