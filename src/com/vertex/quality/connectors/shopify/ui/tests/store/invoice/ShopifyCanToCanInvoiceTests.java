...
/**
 * CCMMER-4066 Shopify - Test automation - Part 5, Contains all Canada to Canada invoice tests
 *
 * @author Shivam.Soni
 */
public class ShopifyCanToCanInvoiceTests extends ShopifyUIBaseTest {
    ...
    /**
     * XRAYSHOP-39 Shopify - Invoice - Test Case -Create Sales Order with Invoice CANQC to CANBC different Province (GST/PST)
     */
    @Test(groups = {"shopify_regression", "shopify_invoice"})
    public void taxInvoiceWithCANQCToCANBCTest() {
        // Load storefront
        loadShopifyStore();
    ...
    /**
     * XRAYSHOP-37 Shopify - Invoice - Test Case -Create Sales Order with Invoice CANBC to CANQC different Province (GST/QST)
     */
    @Test(groups = {"shopify_regression", "shopify_invoice"})
    public void taxInvoiceWithCANBCToCANQCTest() {
        // Load storefront
        loadShopifyStore();
        ...
        storePaymentPage = storeShippingInfoPage.clickContinuePayment();
        tax = storePaymentPage.getTaxFromUIBeforeOrderPlace();
        assertEquals(tax, storePaymentPage.calculatePercentBasedTaxBeforeOrderPlaceUpRounding(14.975));
        storePaymentPage.enterCreditCardDetails();
        orderConfirmationPage = storePaymentPage.clickPayNow();
        uiOrderNo = orderConfirmationPage.getOrderNoFromUI();
        VertexLogger.log(uiOrderNo);
    ...
    /**
     * XRAYSHOP-40 Shopify - Invoice - Test Case -Create Sales Order with Invoice CANNB to CANNB same Province (HST)
     */
    @Test(groups = {"shopify_regression", "shopify_invoice"})
    public void taxInvoiceWithCANNBToCANNBTest() {
        // Load storefront
        loadShopifyStore();
        ...
        storePaymentPage = storeShippingInfoPage.clickContinuePayment();
        tax = storePaymentPage.getTaxFromUIBeforeOrderPlace();
        assertEquals(tax, storePaymentPage.calculatePercentBasedTaxBeforeOrderPlace(15));
        storePaymentPage.enterCreditCardDetails();
        orderConfirmationPage = storePaymentPage.clickPayNow();
        uiOrderNo = orderConfirmationPage.getOrderNoFromUI();
        VertexLogger.log(uiOrderNo);
    ...
    /**
     * XRAYSHOP-38 Shopify - Invoice - Test Case -Create Sales Order with Invoice CANBC to CANON different Province (GST/HST)
     */
    @Test(groups = {"shopify_regression", "shopify_invoice"})
    public void taxInvoiceWithCANBCToCANONTest() {
        // Load storefront
        loadShopifyStore();
