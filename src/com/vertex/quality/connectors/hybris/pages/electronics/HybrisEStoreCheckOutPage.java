 ...
 */
public class HybrisEStoreCheckOutPage extends HybrisBasePage
{
	public HybrisEStoreCheckOutPage( WebDriver driver )
	{
		super(driver);
	}
	...
	protected By SHIPPING_ADDRESS_LINE2 = By.id("address.line2");
	...
	protected By ORDER_DISCOUNTS = By.cssSelector("[class='subtotals ']>[class='subtotals__item--state-discount']");
	...
	protected By ORDER_DISCOUNT_FINAL_REVIEW = By.cssSelector(
		"[class='subtotals dark']>[class='subtotals__item--state-discount']> span");
	...
	/***
	 * method to set address line2
	 *
	 * @param addressLine2
	 *            - Set Optional Address Line2
	 */
	public void setAddressLine_2( String addressLine2 )
	{
		wait.waitForElementDisplayed(SHIPPING_ADDRESS_LINE2);
		text.enterText(SHIPPING_ADDRESS_LINE2, addressLine2);
	}
	...
	public void selectState( String state )
	{
		...
		if(element.isElementPresent(SHIPPING_STATE_DROPDOWN)) {
	...
	public String getShippingMethod( )
	{
		...
		final String selectedOption = ele
			.getText()
			.trim();
	 ...
	 * @return - False if TaxAmount is NOT displayed
	 */
	public boolean isTaxDisplayed( )
	{
		boolean flag = false;
		if ( element.isElementDisplayed(ORDER_SUMMERY_TAX) )
		{
			flag = true;
		}
		return flag;
	...
	public float getTaxAmount( )
	{
		...
		if ( isTaxDisplayed() )
		{
			//			taxAmountStr = getElementAttribute(ORDER_SUMMERY_TAX, "text").trim();
			taxAmountStr = attribute
				.getElementAttribute(ORDER_SUMMERY_TAX, "textContent")
				.trim();
	...
	/***
	 * Method to get Order Discount from Order Summary page
	 *
	 * @return OrderDiscount Amount in Float
	 */
	public float getOrderDiscount( )
	{
		float result = 0;
		if ( element.isElementDisplayed(ORDER_DISCOUNTS) )
		{
			final String orderDiscountStr = attribute
				.getElementAttribute(ORDER_DISCOUNTS, "textContent")
				.trim();
	...
	/***
	 * Method to get Order Discount Amount from Review Section of Order Summary Page
	 *
	 * @return Final Review Order Discount Amount in Float
	 */
	public float getFinalReviewOrderDiscount( )
	{
		float result = 0;
		if ( element.isElementDisplayed(ORDER_DISCOUNT_FINAL_REVIEW) )
		{
			final String orderDiscount = attribute
				.getElementAttribute(ORDER_DISCOUNT_FINAL_REVIEW, "textContent")
				.trim();
			VertexLogger.log("Final Review Order Discount Amount: " + orderDiscount);
			result = Float.parseFloat(orderDiscount.replace("$", ""));
		}
		return result;
	}
	 ...
	 */
	public HybrisEStoreOrderConfirmationPage placeOrder( )
	{
		String heading = "";
		...
		final HybrisEStoreOrderConfirmationPage orderconfirmpage = initializePageObject(
			HybrisEStoreOrderConfirmationPage.class);
	 ...
	 *
	 */
	public HybrisEStorePage addProductToCart( )
	{
		click.clickElement(ADDTOCART_PRODUCTPAGE);
		final HybrisEStorePage storeFront = initializePageObject(HybrisEStorePage.class);
		return storeFront;
	}
	...
	public float calculatePercentageBasedTax(double taxPercent) {
		double tax = (getSubtotal() + getDeliveryAmount() - getOrderDiscount()) * (taxPercent / 100);
