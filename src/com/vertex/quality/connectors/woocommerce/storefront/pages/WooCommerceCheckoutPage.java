...
public class WooCommerceCheckoutPage extends WooCommerceCartPage {
    ...
    protected final By taxAmountAfterOrderPlace = By.xpath(".//th[text()='Tax:']/following-sibling::td/span");
    protected final By subTotalBeforeOrderPlace = By.xpath(".//th[text()='Subtotal']/following-sibling::td/span");
    protected final By subTotalAfterOrderPlace = By.xpath(".//th[text()='Subtotal:']/following-sibling::td/span");
    protected final By shippingBeforeOrderPlace = By.xpath("(.//th[text()='Shipping']/following-sibling::td//span)[1]");
    protected final By shippingAfterOrderPlace = By.xpath(".//th[text()='Shipping:']/following-sibling::td/span");
    protected final By couponBeforeOrderPlace = By.xpath(".//th[contains(text(),'Coupon:')]/following-sibling::td/span");
    ...
    public String getAddressCleansingMessage() {
        String acMSG;
        jsWaiter.sleep(1000);
        WebElement msg = wait.waitForElementPresent(addressCleansingPopup);
        acMSG = text.getElementText(msg);
        VertexLogger.log(acMSG);
        return acMSG;
    }
    ...
    public String getTaxFromUIAfterOrderPlace() {
        waitForPageLoad();
        wait.waitForElementPresent(orderReceivedLabel);
        return text.getElementText(taxAmountAfterOrderPlace);
    }
     ...
     * @return total tax value of order
     */
    public double getTaxFromUIAfterOrder() {
        waitForPageLoad();
        wait.waitForElementPresent(orderReceivedLabel);
        VertexLogger.log("UI: " + text.getElementText(taxAmountAfterOrderPlace));
        return Double.parseDouble(text.getElementText(taxAmountAfterOrderPlace).replace("$", ""));
    }
    ...
    public double calculatePercentBasedTax(double taxPercent) {
        ...
        double total = (Double.parseDouble(text.getElementText(subTotalAfterOrderPlace).replace("$", "")) + shipping - discount)
                * (taxPercent / 100);
        VertexLogger.log("Calculated: " + total);
     ...
     * @return calculated tax amount based on percentage
     */
    public double calculatePercentBasedTaxBeforeOrderPlace(double taxPercent) {
        double shipping = 0;
        double coupon = 0;
        if (element.isElementDisplayed(shippingBeforeOrderPlace)) {
            shipping = Double.parseDouble(text.getElementText(shippingBeforeOrderPlace).replace("$", ""));
        }
        if (element.isElementDisplayed(couponBeforeOrderPlace)) {
            coupon = Double.parseDouble(text.getElementText(couponBeforeOrderPlace).replace("$", ""));
        }
        double total = (Double.parseDouble(text.getElementText(subTotalBeforeOrderPlace).replace("$", "")) + shipping - coupon)
                * (taxPercent / 100);
        VertexLogger.log("Calculated: " + total);
        return Double.parseDouble(String.format("%.2f", total));
    }
    ...
    public String getTaxFromUIBeforeOrderPlace() {
        ...
        VertexLogger.log("UI: " + text.getElementText(taxAmountBeforeOrderPlace));
     ...
     * @return total tax value of order
     */
    public double getTaxFromUIBeforeOrder() {
        wait.waitForElementPresent(taxAmountBeforeOrderPlace);
        VertexLogger.log("UI: " + text.getElementText(taxAmountBeforeOrderPlace).replace("$", ""));
        return Double.parseDouble(text.getElementText(taxAmountBeforeOrderPlace).replace("$", ""));
    }
