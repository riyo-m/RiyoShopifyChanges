 ...
 * @author
 */
public class SalesForceCRMAddressesPage extends SalesForceBasePage
{
	...
	protected By VALIDATE_ADDRESS_CANCEL_BUTTON = By.xpath(
		"//button[contains(text(),'Cancel') and not(ancestor::*[contains(@style,'display:none')]) and not(ancestor::*[contains(@style,'display: none')])]");
	protected By PHYSICALORIGIN_VALIDATE_ADDRESS_BUTTON_OLD = By.xpath(
		"//button[@onclick=\"validateAddress('Default Physical Origin Address');return false;\"][2]");
	protected By PHYSICAL_ORIGIN_VALIDATE_ADDRESS_BUTTON = By.xpath(
		".//button[@name='Default Physical Origin Address' and @title = 'Validate Address']");

	protected By ADDRESS_VALIDATION_ERROR_MESSAGE = By.xpath(".//label[contains(@id, 'errorMsgDisplay')]");

	public SalesForceCRMAddressesPage( WebDriver driver )
	{
		super(driver);
	}
	...
	public void clickButtonsOnAddressTab( String addressSection, String buttonName )
	{
		...
		wait.waitForElementEnabled(addressButton);
	...
	{
		click.clickElement(EDIT_SAVE_BUTTON);
	...
	/**
	 * Validate Address Button does'nt Exits
	 *
	...
	public boolean VerifyValidateAddressButton( )
	{
		...
		validateAddress = element.isElementDisplayed(VALIDATE_ADDRESS_BUTTON);
	...
	public void clickAssignTaxToAddressButton( )
	{
		...
		wait.waitForElementDisplayed(ASSIGN_TAX_ID_TO_ADDRESS_BUTTON);
	...
	/**
	 * Click on Cancel button in Validate Address pop up
	 */
	public void clickCancelButton( )
	{
		waitForSalesForceLoaded();
		scroll.scrollElementIntoView(VALIDATE_ADDRESS_CANCEL_BUTTON);
		wait.waitForElementDisplayed(VALIDATE_ADDRESS_CANCEL_BUTTON);
		click.clickElement(VALIDATE_ADDRESS_CANCEL_BUTTON);
		waitForPageLoad();
	}

	/**
	 * Click on Validate Address button in Address page
	 */
	public void clickPhysicalOriginValidateAddressButton( )
	{
		By uiElement = PHYSICAL_ORIGIN_VALIDATE_ADDRESS_BUTTON;
		if ( !element.isElementPresent(uiElement) )
		{
			uiElement = PHYSICALORIGIN_VALIDATE_ADDRESS_BUTTON_OLD;
		}
