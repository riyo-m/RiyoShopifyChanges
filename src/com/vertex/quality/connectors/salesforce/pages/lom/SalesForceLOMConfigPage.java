 ...
 */
public class SalesForceLOMConfigPage extends SalesForceLB2BPostLogInPage
{
	...
	public SalesForceLOMConfigPage( WebDriver driver )
	{
		super(driver);
	}
	...
	{
		VertexLogger.log("Clicking SaveAndValidate button...", VertexLogLevel.TRACE, SalesForceLOMConfigPage.class);
	...
	public void clickSpecificLogNameLink( String logDetails )
	{
		...
		String logNameLink = String.format(
			...
			logDetails);
		wait.waitForElementDisplayed(By.xpath(logNameLink));
//		if ( !element.isElementPresent(By.xpath(logNameLink)) )
//		{
//			logNameLink = String.format(
//				".//*[@id='table-container-custom']/div/table/tbody/tr/td/div[text()='%s']/parent::td/parent::tr/th/div/a",
//				logDetails);
//		}

		wait.waitForElementDisplayed(By.xpath(logNameLink));
		click.clickElement(By.xpath(logNameLink));

		window.switchToWindowTextInTitle("VL");
	}

	/**
	 * Click on most recent Log Name link with matching Transaction ID section to validate the request and response
	 * @param transactionId
	 */
	public void clickSpecificLogNameLinkBasedOnTransactionId(String transactionId)
	{
		...
		boolean newWindow = true;
		String locator = String.format("(.//lightning-datatable//td[@data-label='Transaction Id']//div/lightning-formatted-url/a[starts-with(text(),'%s')]/ancestor::tr[@class='slds-hint-parent']/th[@data-label='Log Name']//lightning-formatted-url)[1]", transactionId);
		By logElement = By.xpath(locator);
		wait.waitForElementDisplayed(logElement);
		click.javascriptClick(logElement);
	...
	public void clickClearTaxDetailButton() {
		...
		wait.waitForElementDisplayed(CLEAR_TAX_DETAIL_BUTTON);
