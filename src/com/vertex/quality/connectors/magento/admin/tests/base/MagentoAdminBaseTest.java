package com.vertex.quality.connectors.magento.admin.tests.base;
...
public abstract class MagentoAdminBaseTest extends M2StorefrontBaseTest {
    ...
    protected String username = MagentoData.MAGENTO_2_0_AUTOMATION_USER_ADMIN_PANEL.data;
    protected String password = MagentoData.MAGENTO_2_0_ADMIN_PASSWORD.data;
    ...
    protected String customerID = "Test Test";
    ...
    protected M2AdminHomepage signInToAdminHomepage() {
        ...
        signOnPage.enterUsername(MagentoData.MAGENTO_2_0_AUTOMATION_USER_ADMIN_PANEL.data);

        signOnPage.enterPassword(MagentoData.MAGENTO_2_0_ADMIN_PASSWORD.data);
     ...
     * @return the Customers Page
     */
    protected M2AdminCustomersPage navigateToAllCustomersWithoutSignIn() {
        M2AdminHomepage homePage = new M2AdminHomepage(driver);
        homePage.navPanel.clickCustomersButton();
        return homePage.navPanel.clickAllCustomersButton();
    }
    ...
    protected M2AdminCreateNewOrderPage addMultipleLineProductDetails(String coupon) {
        ...
        newOrderPage.clickAddSelectedProductToOrderButton();
        newOrderPage.enterCouponCode(coupon);
        newOrderPage.clickApplyDiscountButton();
        newOrderPage.clickAddProducts();
     ...
     * @return the Order Customers Page
     */
    protected M2AdminCreateNewOrderPage addProductAddressDetailsForCustomerCodeClass(String qty) {
        M2AdminOrdersPage ordersPage = navigateToOrdersWithoutSignIn();
        M2AdminOrderCustomerPage orderCustomerPage = ordersPage.clickNewOrderButton();
        M2AdminCreateNewOrderPage newOrderPage = orderCustomerPage.clickCustomer(MagentoData.CUSTOMER_FIRST_NAME.data + " " + MagentoData.CUSTOMER_LAST_NAME.data);
        newOrderPage.selectDefaultStore();
        newOrderPage.clickAddProducts();
        newOrderPage.enterSKUNumber(SKU);
        newOrderPage.enterNewQty(SKU, qty);
     ...
     * @return the Order Customers Page
     */
    protected M2AdminCreateNewOrderPage addProductAddressDetailsForCustomerCodeClass(String customerName, String qty) {
        M2AdminOrdersPage ordersPage = navigateToOrdersWithoutSignIn();
        M2AdminOrderCustomerPage orderCustomerPage = ordersPage.clickNewOrderButton();
        M2AdminCreateNewOrderPage newOrderPage = orderCustomerPage.clickCustomer(customerName);
        newOrderPage.selectDefaultStore();
        newOrderPage.clickAddProducts();
        newOrderPage.enterSKUNumber(SKU);
        newOrderPage.enterNewQty(SKU, qty);
    ...
    /**
     * tests whether customer can add product and add address without navigate to order without signIn.
     *
     * @param sku SKU code of the product
     * @param qty quantity
     * @return the Order Customers Page
     */
    protected M2AdminCreateNewOrderPage addProductsAddressDetails(String sku, String qty) {
        M2AdminOrdersPage ordersPage = navigateToOrdersWithoutSignIn();
        M2AdminOrderCustomerPage orderCustomerPage = ordersPage.clickNewOrderButton();
        M2AdminCreateNewOrderPage newOrderPage = orderCustomerPage.clickCustomer(customerID);
        newOrderPage.selectDefaultStore();
    ...
    /**
     * Add multiple products & enter respective quantity or quantities
     *
     * @param loginStatus Login status
     * @param products    names of multiple products
     * @param qty         quantity of the products
     * @return the Order Customers Page
     */
    protected M2AdminCreateNewOrderPage addMultipleProductDetails(String loginStatus, String[] products, String[] qty) {
        if (products.length != qty.length) {
            Assert.fail("Products array & Qty array should have same length");
        }
        M2AdminOrdersPage ordersPage;
        if(loginStatus.equals("loggedOut")) {
            ordersPage = navigateToOrders();
        ...
        if (!qty[0].equals("0")) {
            newOrderPage.clickAddSKUButton();
            newOrderPage.enterSKUDetails(products[0], skuField);
            newOrderPage.enterMultiQty(qty[0], qtyField);
        }
        if (!qty[1].equals("0")) {
            newOrderPage.clickAddAnotherButton();
            newOrderPage.enterSKUDetails(products[1], skuField2);
            newOrderPage.enterMultiQty(qty[1], qtyField2);
        }
        if (!qty[2].equals("0")) {
            newOrderPage.clickAddAnotherButton();
            newOrderPage.enterSKUDetails(products[2], skuField3);
            newOrderPage.enterMultiQty(qty[2], qtyField3);
        }
        newOrderPage.clickAddToOrderButton();
        return newOrderPage;
    }
    ...
    protected M2AdminCreateNewOrderPage addProductAddressDetailsForAddressCleansing() {
        ...
        newOrderPage.clickAddProducts();
        newOrderPage.enterSKUNumber(SKU);
        newOrderPage.enterNewQty(SKU, MagentoData.quantity_one.data);
        newOrderPage.clickAddToOrderButton();
     ...
     * @param adjustFee adjusted amount
     * @return Order View Info Page
     */
    protected M2AdminOrderViewInfoPage submitCreditMemoWithRefund(String[] qty, String adjustFee) {
        M2AdminCreditMemoPage creditMemoPage = navigateToNewMemoPageWithoutInvoice();
        creditMemoPage.addAdjustmentRefund(qty[0], qty[1], qty[2], adjustFee);
        return creditMemoPage.clickRefundOfflineButton();
    }
     ...
     * @return Credit Order page
     */
    protected M2AdminCustomerInformationPage setCustomerCode(String customerCode) {
        M2AdminCustomersPage customerPage = navigateToAllCustomersWithoutSignIn();
        ...
        customerInfo.inputVertexCustomerCode(customerCode);
     ...
     * @return Credit Order page
     */
    protected M2AdminCustomerInformationPage setCustomerClass(String customerClass) {
        M2AdminCustomersPage customerPage = navigateToAllCustomersWithoutSignIn();
        M2AdminCustomerInformationPage customerInfo = customerPage.editCustomerByName(MagentoData.CUSTOMER_FIRST_NAME.data, MagentoData.CUSTOMER_LAST_NAME.data);
        customerInfo.clickAccountInformationTab();
        customerInfo.selectCustomerGroup(customerClass);
        customerInfo.clickSaveButton();
        return customerInfo;
    }
}