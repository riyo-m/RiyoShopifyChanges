...
public class M2StorefrontBannerPage extends MagentoStorefrontPage {
    ...
    protected By viewAndCartLink = By.xpath(".//a[@class='action viewcart']");
    protected By customerDropdown = By.xpath("(.//button[normalize-space(.)='Change'])[1]");
    protected By expandedCustomerDropdown = By.xpath("(.//button[normalize-space(.)='Change'])[1]/parent::span[@aria-expanded='true']");
    protected By customerMenuDialog = By.xpath("(.//ul[@class='header links'])[2]");
    protected By customerMenuOptions = By.xpath("(.//ul[@class='header links'])[2]//li");
    protected By signOutOption = By.xpath("(.//ul[@class='header links'])[2]//li[normalize-space(.)='Sign Out']");
    ...
    /**
     * Signs out user from the Magento Store
     */
    public void signOutFromStore() {
        waitForPageLoad();
        waitForSpinnerToBeDisappeared();
        WebElement customerMenu = wait.waitForElementPresent(customerDropdown);
        if (!element.isElementDisplayed(expandedCustomerDropdown)) {
            click.moveToElementAndClick(customerMenu);
            wait.waitForElementPresent(expandedCustomerDropdown);
            wait.waitForElementPresent(customerMenuDialog);
            wait.waitForAllElementsPresent(customerMenuOptions);
        }
