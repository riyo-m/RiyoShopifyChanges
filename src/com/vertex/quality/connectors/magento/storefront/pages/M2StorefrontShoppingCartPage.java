...
public class M2StorefrontShoppingCartPage extends MagentoStorefrontPage
{
	...
	protected By shoppingCartLabel = By.xpath(".//h1[normalize-space(.)='Shopping Cart']");
	protected String cartItemBasePrice = ".//a[@title='<<text_replace>>']/parent::td/following-sibling::td[@class='col price']";
	protected String cartItemQty = ".//a[@title='<<text_replace>>']/parent::td/following-sibling::td[@class='col qty']//input";
	protected String cartItemSubtotal = ".//a[@title='<<text_replace>>']/parent::td/following-sibling::td[@class='col subtotal']";
	protected By updateCartButton = By.xpath(".//button[normalize-space(.)='Update Shopping Cart']");
	...
	/**
	 * clicks the Zip Code textbox and inputs a zipcode
	 */
	...
	/**
	 * Returns the base price of an Item in the cart (Base Price = price of 1 quantity only or decided fix price of product excl. tax)
	 *
	 * @param itemName Product name of which base price to be taken
	 * @return base price of an item
	 */
	public double getItemBasePrice(String itemName) {
		waitForPageLoad();
		waitForSpinnerToBeDisappeared();
		wait.waitForElementPresent(shoppingCartLabel);
		WebElement basePrice = wait.waitForElementPresent(By.xpath(cartItemBasePrice.replace("<<text_replace>>", itemName)));
		return Double.parseDouble(text.getElementText(basePrice).replace("$", "").replace(",", ""));
	}

	/**
	 * Returns the Subtotal of an Item in the cart (Subtotal = Base Price * Quantities excl. of tax)
	 *
	 * @param itemName Product name of which subtotal to be taken
	 * @return subtotal of an item
	 */
	public double getItemSubtotal(String itemName) {
		waitForPageLoad();
		waitForSpinnerToBeDisappeared();
		wait.waitForElementPresent(shoppingCartLabel);
		WebElement subtotal = wait.waitForElementPresent(By.xpath(cartItemSubtotal.replace("<<text_replace>>", itemName)));
		return Double.parseDouble(text.getElementText(subtotal).replace("$", "").replace(",", ""));
	}

	/**
	 * Update Cart item's quantity
	 *
	 * @param itemName Product name of which quantity should be updated
	 * @param quantity No of quantity
	 */
	public void updateItemQty(String itemName, String quantity) {
		waitForPageLoad();
		waitForSpinnerToBeDisappeared();
		wait.waitForElementPresent(shoppingCartLabel);
		WebElement qty = wait.waitForElementPresent(By.xpath(cartItemQty.replace("<<text_replace>>", itemName)));
		text.enterText(qty, quantity);
		text.pressTab(qty);
		updateCart();
	}

	/**
	 * Clicks on Update Shopping Cart button & save the changes made in the cart
	 */
	public void updateCart() {
		waitForPageLoad();
		waitForSpinnerToBeDisappeared();
		wait.waitForElementPresent(shoppingCartLabel);
		click.moveToElementAndClick(wait.waitForElementPresent(updateCartButton));
	}
}