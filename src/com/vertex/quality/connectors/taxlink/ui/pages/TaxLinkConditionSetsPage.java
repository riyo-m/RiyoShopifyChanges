...
public class TaxLinkConditionSetsPage extends TaxLinkBasePage
{
	public TaxLinkConditionSetsPage( final WebDriver driver ) throws IOException
	{
		super(driver);
	...
	private WebElement viewButtonConditionSetPage;

	@FindBy(xpath = "//button[contains(text(), 'Add')]")
	private WebElement addConditionButton;

	@FindBy(xpath = "//select[@data-cy='conditionSet-conditionSetScope']")
	private WebElement condSetScopeDropdown;
	...
	@FindBy(xpath = "//div[contains(@class,'ag-cell ag-cell-not-inline-editing ag-cell-with-height grid-action-cell ag-cell-value')][@col-id='action-col']")
	private List<WebElement> editColCondPresentation;
	...
	@FindBy(xpath = "//div[@ref='btPrevious']/span")
	private WebElement previousPageConditionSet;
	...
	/**
	 * Verify title of Condition Sets Page in Taxlink application
	 *
	 * @return
	 */
	public boolean verifyTitleConditionSetsPage( )
	{
		boolean titleMatch = false;
		String conditionSetsTitle;
		wait.waitForElementDisplayed(summaryPageTitle);
		conditionSetsTitle = summaryPageTitle.getText();
		if ( conditionSetsTitle.equalsIgnoreCase(RulesMapping.RuleMappingDetails.headerConditionSetPage) )
		{
			titleMatch = true;
		}
		return titleMatch;
	...
	public boolean verifyTitleAddConditionSetPage( )
	{
		...
		String addCondSetTitle = wait
			...
			.getText();
		if ( addCondSetTitle.equalsIgnoreCase(RulesMapping.RuleMappingDetails.headerAddConditionSetPage) )
		{
	...
	/**
	 * Method to title of view page of Condition Set
	 *
	 * @return boolean
	 */
	public boolean verifyTitleViewConditionSetPage( )
	{
		boolean flagViewCondTitleMatches = false;
		String viewCondSetTitle = wait
			.waitForElementDisplayed(viewCondSetPageTitle)
			.getText();
		if ( viewCondSetTitle.equalsIgnoreCase(RulesMapping.RuleMappingDetails.headerViewConditionSetPage) )
		{
	...
	/**
	 * Method to title of edit page of Condition Set
	 *
	 * @return boolean
	 */
	public boolean verifyTitleEditConditionSetPage( )
	{
		boolean flagEditCondTitleMatches = false;
		String editCondSetTitle = wait
			.waitForElementDisplayed(addViewEditPageTitle)
			.getText();
		if ( editCondSetTitle.equalsIgnoreCase(RulesMapping.RuleMappingDetails.headerEditConditionSetPage) )
		{
	 ...
	 * @return boolean
	 */
	public boolean verifyViewPageOfConditionSet( )
	{
		boolean flagVerifyViewCondSetPage = false;
		if ( verifyTitleViewConditionSetPage() )
		{
			boolean condNameViewCondSetPageFlag = !condNameCondSetPage.isEnabled();
			VertexLogger.log(String.valueOf(condNameViewCondSetPageFlag));
			boolean viewScopeCondSetPageFlag = !condSetScopeDropdown.isEnabled();
			VertexLogger.log(String.valueOf(viewScopeCondSetPageFlag));
			boolean stDateViewCondSetPageFlag = !startDate.isEnabled();
			VertexLogger.log(String.valueOf(stDateViewCondSetPageFlag));
			boolean endDateViewCondSetPageFlag = !endDate.isEnabled();
			VertexLogger.log(String.valueOf(endDateViewCondSetPageFlag));
			boolean enabledViewCondSetPageFlag = !enabledFlag.isEnabled();
			VertexLogger.log(String.valueOf(enabledViewCondSetPageFlag));
			boolean saveViewCondSetPageFlag = !saveButton.isEnabled();
			VertexLogger.log(String.valueOf(saveViewCondSetPageFlag));
			boolean cancelViewCondSetPageFlag = cancelButton.isEnabled();
			VertexLogger.log(String.valueOf(cancelViewCondSetPageFlag));
			if ( condNameViewCondSetPageFlag && viewScopeCondSetPageFlag && stDateViewCondSetPageFlag &&
				 endDateViewCondSetPageFlag && enabledViewCondSetPageFlag && saveViewCondSetPageFlag &&
				 cancelViewCondSetPageFlag )
			{
				flagVerifyViewCondSetPage = true;
			}
			else
			{
				flagVerifyViewCondSetPage = false;
			}
		}
		return flagVerifyViewCondSetPage;
	}
	...
	/**
	 * View Condition Sets with System level access type in Rules Mapping
	 * in Taxlink application
	 *
	 * @return boolean
	 */
	public boolean viewSYSTEMConditionSetsRulesMapping( )
	{
		final AtomicBoolean flagViewSYSTEMConditionSets = new AtomicBoolean(false);

		navigateToConditionSets();
		verifyTitleConditionSetsPage();
		List<WebElement> list = new ArrayList<>(conditionSetNamePresentation);
		List<String> stringsList = new ArrayList<String>();

		for ( WebElement elements : list )
		{
			stringsList.add(elements.getText());
		}
		...
		for ( int i = 1 ; i <= count ; i++ )
		{
			for ( int l = 0 ; l < accessTypeConditionSetPresentation.size() ; l++ )
			{
				...
				if ( accessTypeConditionSetPresentation
					...
					.get(l)
					.getText()).contains("ONLY") || (conditionSetNamePresentation
					.get(l)
					.getText()).contains("DEFAULT CONDITION SET") )
				{
	 ...
	 * @return boolean
	 */
	public boolean addAndVerifyConditionSetWithEndDate( )
	{
		...
		if ( verifyTitleAddConditionSetPage() )
		{
			...
			dropdown.selectDropdownByValue(condSetScopeDropdown, "ALL");
			String condSetScope = condSetScopeDropdown.getAttribute("value");
			...
			for ( int c = 1 ; c < 6 ; c++ )
			{
				...
				switch ( selectedOp )
				{
					case "EQUAL_TO":
						...
						dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Application ID");
					...
					case "LESS_THAN_OR_EQUAL_TO":
						wait.tryWaitForElementEnabled(conditionSourceDropdown, 20);
						dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Bill To TAID");
						source = dropdown.getDropdownSelectedOption(conditionSourceDropdown);
						selectedSrc = source.getText();
					...
					case "NOT_IN":
						wait.tryWaitForElementEnabled(conditionSourceDropdown, 20);
						dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Bill From TAID");
						source = dropdown.getDropdownSelectedOption(conditionSourceDropdown);
						selectedSrc = source.getText();
					...
					case "IN":
						wait.tryWaitForElementEnabled(conditionSourceDropdown, 20);
						dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Application Short Name");
						source = dropdown.getDropdownSelectedOption(conditionSourceDropdown);
						selectedSrc = source.getText();
						if ( negativeScenarioForCommaValues(utils.randomNumber("4")) )
						{
					...
					case "IS_NULL":
						wait.tryWaitForElementEnabled(conditionSourceDropdown, 20);
						dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Batch Source Name");
						source = dropdown.getDropdownSelectedOption(conditionSourceDropdown);
						selectedSrc = source.getText();
						break;
					default:
	 ...
	 * @return boolean
	 */
	public boolean addAndVerifyConditionSetWithoutEndDate( )
	{
		...
		jsWaiter.sleep(2000);
		js.executeScript("arguments[0].scrollIntoView();", conditionSetsButton);
		wait.waitForElementDisplayed(conditionSetsButton);
	...
	/**
	 * View Condition Sets with user level access type in Rules Mapping
	 * in Taxlink application
	 *
	 * @return boolean
	 */
	public boolean viewUSERConditionSetsRulesMapping( )
	{
	...
	/**
	 * Edit Condition Sets with user level access type in Rules Mapping
	 * in Taxlink application
	 *
	 * @return boolean
	 */
	public boolean editUSERConditionSetsRulesMapping( )
	{
		...
		navigateToConditionSets();
		verifyTitleConditionSetsPage();
		if ( verifyTitleConditionSetsPage() )
		{
	...
	public boolean verifyViewConditionUser( )
	{
		...
		if ( verifyTitleViewConditionSetPage() )
		{
			...
			if ( condNameViewCondSetPageFlag && stDateViewCondSetPageFlag && endDateViewCondSetPageFlag &&
			...
			{
				flagViewConditionForUser = false;
			}
		}
		...
		for ( int i = 1 ; i <= countSummaryTable ; i++ )
		{
			...
			for ( int j = 0 ; j < accessTypeConditionSetPresentation.size() ; j++ )
			{
					...
					.equals("Y")) )
				{
					...
					jsWaiter.sleep(5000);
					js.executeScript("arguments[0].scrollIntoView();", conditionSetNamePresentation
						.get(j)
						.findElement(condSetNamePresentation));
					conditionSetNamePresentation
						.get(j)
	...
	public boolean verifyEditConditionSetUser( )
	{
		...
		if ( verifyTitleEditConditionSetPage() )
		{
				...
				.equals("") )
			{
				...
				if ( popUpConditionUpdate.isDisplayed() )
				{
					if ( messageOnEditSaveCondition
						.getText()
						.equals(RulesMapping.RuleMappingDetails.messageEditEndDateConditionSetPage) )
					{
	...
	public boolean disabledConditionSetFunctionality( String condSelected )
	{
		...
		if ( popUpConditionUpdate.isDisplayed() )
		{
			if ( messageOnEditSaveCondition
				.getText()
				.equals(RulesMapping.RuleMappingDetails.messageEditConditionSetPage) )
			{
				click.clickElement(yesOnEditSaveConditionPopUp);
			}
		}
	...
	/**
	 * Export to CSV on Condition Sets page
	 *
	 * @param instName
	 * @return boolean
	 */

	public boolean exportToCSVConditionSets( String instName ) throws IOException
	{
		...
		String fileName = "ConditionSets_Extract.csv";
				...
				for ( CSVRecord csvRecord : csvParser )
				{
					...
					if ( !conditionName_CSV.equals("Condition Name") )
					{
						...
						if ( dataConditionName.isPresent() )
						{
							...
							Optional condSetScope = dataPresentInParticularColumn(condSetScopePresentation, scope_CSV);
	...
	/**
	 * Verify Rules tied to the Default Condition Set in Tax link application
	 *
	 * @param ruleName
	...
	public boolean verifyRulesForDefaultConditionSetTest( String ruleName, String phaseTypeOfRule )
	{
		...
		for ( int i = 1 ; i <= count ; i++ )
		{
			...
			if ( flagConditionPresence.isPresent() )
			{
				...
				detailsButton.click();
				VertexLogger.log("Clicked on Details button for the Condition set: VTX DEFAULT CONDITION SET");
				break;
			}
			else
			{
		...
		try
		{
			if ( summaryPageTitle
				.getText()
				.equals("Condition VTX DEFAULT CONDITION SET Details") )
			{
				...
				for ( int i = 1 ; i <= pageCount ; i++ )
				{
					...
					if ( flagRulePresence.isPresent() )
					{
						...
						{
							VertexLogger.log(
								"Rule verified in the details tab for condition set: VTX DEFAULT CONDITION SET");
							finalFlag = true;
							break;
						}
					}
	 ...
	 * @return conditionSet
	 */
	public String addAndVerifyConditionSet( )
	{
		String value = null;
		...
		if ( verifyTitleAddConditionSetPage() )
		{
			...
			dropdown.selectDropdownByDisplayName(conditionSourceDropdown, "Vendor Number");
			source = dropdown.getDropdownSelectedOption(conditionSourceDropdown);
			selectedSrc = source.getText();
	...
	/**
	 * Verify Rules tied to the User defined Condition Set in Tax link application
	 *
	 * @param ruleName
	...
	public boolean verifyRulesForUserDefinedConditionSetTest( String conditionSetName, String ruleName,
		String phaseTypeOfRule )
	{
		...
		try
		{
			if ( summaryPageTitle
				.getText()
				.equals("Condition " + conditionSetName + " Details") )
			{
				int pageCount = Integer.parseInt(totalPageCountSummaryTable.getText());
				for ( int i = 1 ; i <= pageCount ; i++ )
				{
	...
	public void clickOnPostCalcRules( )
	{
	...
	public void clickOnPreCalcRules( )
	{
	...
	/**
	 * Copy Condition Sets in Taxlink application
	 *
	 * @return boolean
	 */
	public boolean copyUSERConditionSetsRulesMapping( )
	{
	...
	public boolean verifyCopyConditionUser( )
	{
		...
		if ( verifyTitleCopyConditionSetPage() )
		{
			...
			if ( popUpConditionUpdate.isDisplayed() )
			{
				if ( messageOnCopySaveCondition
					.getText()
					.equals(RulesMapping.RuleMappingDetails.messageCopyConditionSetPage) )
				{
					click.clickElement(yesOnEditSaveConditionPopUp);
				}
			}
	...
	/**
	 * Method to verify title of view page of Condition Set
	 *
	 * @return boolean
	 */
	public boolean verifyTitleCopyConditionSetPage( )
	{
		boolean flagCopyCondTitleMatches = false;
		String copyCondSetTitle = wait
			.waitForElementDisplayed(copyCondSetPageTitle)
			.getText();
		if ( copyCondSetTitle.equalsIgnoreCase(RulesMapping.RuleMappingDetails.headerCopyConditionSetPage) )
		{
	...
	public int getRulesCountForUserDefinedConditionSetTest( String conditionSetName )
	{
		int rulesCount = 0;
	...
	/**
	 * Verify Rules tied to the User copied Condition Set in Tax link application
	 *
	 * @param ruleName
	 * @param conditionSetName
	 *
	...
	public boolean verifyRulesForUserCopiedConditionSetTest( String conditionSetName, String ruleName )
	{
		boolean finalFlag = false;
	...
	/**
	 * Method to check when a condition set is disabled,
	 * all associated Pre calc rules tied to that condition set gets disabled
	 *
	 * @param preCalcRuleName
	 * @return boolean
	 */
	public boolean verifyPreCalcDisabledRulesForDisabledConditionSet( String preCalcRuleName )
	{
	...
	/**
	 * Method to check when a condition set is disabled,
	 * all associated Post calc rules tied to that condition set gets disabled
	 *
	 * @param postCalcRuleName
	 * @return boolean
	 */
	public boolean verifyPostCalcDisabledRulesForDisabledConditionSet( String postCalcRuleName )
	{
		boolean flagRuleDisabledVerified = false;
		...
		for ( int i = 1 ; i <= countSummaryTable ; i++ )
		{
			Optional<WebElement> dataFound = dataPresentInParticularColumn(ruleNamePresentation, postCalcRuleName);
			if ( dataFound.isPresent() )
			{
				if ( dataFound
					.get()
				...
				{
					flagRuleDisabledVerified = true;
					VertexLogger.log("Post-calc rule has been disabled for the condition set");
					break;
				}
				else
				{
					VertexLogger.log("Post-calc rule is still enabled for the condition set");
				}
			}
			else
			{
