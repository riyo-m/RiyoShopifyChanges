...
/**
 * A generic Oracle ERP SOAP API.
 *
 * @author Aman Jain
 */
public abstract class OracleSoapApi
{
    ...
    public SOAPMessage addAuthHeader(SOAPMessage localMessage) {
        try {
            ...
            token.setUserInfo(settings.oracle_username, settings.oracle_password);
            token.addNonce();
            token.addCreated();
            token.build();
	...
	/**
	 * Obtain documentId from the response
	 *
	 * @return docId obtained from the SOAP Response
	 */
	public String getDocumentIdFromResponse( )
	{
		String documentId = "";
		try
		{
			SOAPBody soapBody = responseAsSoap.getSOAPBody();
			documentId = soapBody.getTextContent();
		}
		catch ( SOAPException se )
		{
			VertexLogger.log("SOAP error. Please refer to your request", VertexLogLevel.ERROR);
