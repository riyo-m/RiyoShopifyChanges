...
import com.vertex.quality.connectors.taxlink.ui_e2e.pages.TaxLinkJournalInvOutputFilePage;
...
/**
 * this class represents the smoke test case for Journal Output File
 * in Inventory Rules Mapping Module Tab containing in Taxlink UI.
 *
 * @author mgaikwad
 */

public class TaxLinkJournalInvOutputFileTests extends TaxLinkBaseTest
{
	...
	public void loginTestCase( ) throws IOException
	{
		...
		journalInvOutputFilePage = new TaxLinkJournalInvOutputFilePage(driver);
	...
	public void addAndVerifyMapFunctionJournalRule_e2e_Test( ) throws IOException
	{
		assertTrue(homePage.selectInstance(OnboardingDashboard.INSTANCE_DETAILS.instanceNameWithDC));
		ruleID = journalInvOutputFilePage.addAndVerifyInvJournalRule("MAP",
			InventoryRulesMapping.INVENTORY_RULES_MAPPING.invConditionSet, "Transaction type name", "GL Attribute01");
		writeToFile(ruleID);
	}

	/**
	 * Verify taxlink database for Journal output for inv rules mapping
	 * after creating a request in Oracle's UCM server
	 * Map Function
	 * COERPC-11122
	 */
	@Test(groups = { "taxlink_invrulesmapping_regression" })
	public void verifyDBJournalInvRuleMapFunction_e2e_Test( ) throws Exception
	{
		...
		assertTrue(dbPage.db_verifyMapJournalInvRuleInDB(fusionInstanceID, jobID));
	}
}
