...
public class EpiCheckoutDifferentBillingShippingAddressTests extends EpiBaseTest
{
	@Test(groups = { "smoke" })
	public void episerverDifferentBillingAddressTest( )
	{
		...
		EpiOrderHistoryPage orderhistorypage;
		...
		// Select And Validate newly added Billing and Shipping Address
		String BAddress = Address.Tysons.city;
		checkoutpage.selectBillingAddress(BAddress);
		checkoutpage.validateBillingAddressSelected(BAddress);
		String SAddress = Address.Edison.city;
		checkoutpage.selectBillingAddress(SAddress);
		checkoutpage.validateBillingAddressSelected(SAddress);
		...
		addresspage.deleteAddress("SHIPPINGADDRNJ", true);

		// Validate the Order ID and Price on Order History Page
		portalhomepage.navigateToHomePage();
		portalhomepage.clickUserIcon();
		orderhistorypage = portalhomepage.clickOrderHistory();
		// Validate final cost after order placed
		String actualcost = orderhistorypage.getPrice(orderNumber);
		String expectedcost = Double.toString(CART_TOTAL);
		assertEquals(actualcost, expectedcost);
		VertexLogger.log(
			String.format(("Actual cost :%s is matched with expected cost on Order History page: %s"), actualcost,
				expectedcost));
